...
`,
    'SLIDEBOOK.md': `# Vendor Integration: 3i SlideBook

This document provides examples of how complex, multi-step workflows in vendor-specific software like **3i SlideBook** are simplified into single, readable CUBE commands.

## Example: Batch Processing

### Before: SlideBook UI / XML Script
A typical batch process might involve loading a protocol file, executing a series of acquisition and analysis steps, and exporting the results. This is often configured through a graphical user interface or a verbose XML-based script.

### After: CUBE Protocol
The entire workflow is reduced to a single, self-documenting command.

\`\`\`cube
SLIDEBOOK|BATCH[Load:Experiment_Protocol.sb]→EXECUTE→EXPORT[OME-TIFF]|AUTOMATED
\`\`\`

This demonstrates the power of CUBE to abstract complexity and create shareable, reproducible workflows.
`,
    'TECHNICAL.md': `# Technical Workflows

This section provides examples of how common, technically complex microscopy workflows are represented in the CUBE Protocol.

## Example: GPU-Accelerated Super-Resolution

### Before: 500+ lines of Python
Implementing a state-of-the-art super-resolution model like SRDTrans involves complex Python code, environment setup, and GPU management. (See the 'AI Model Integration' tab for a full breakdown).

### After: CUBE Protocol
The entire process is a single command.

\`\`\`cube
# SRDTrans: 500+ lines of Python -> 1 CUBE command
ENHANCE|IMAGE[input.tif]→SUPER_RES[SRDTrans:4x]→DENOISE→SAVE|HD
\`\`\`

## Example: Live Cell Tracking with Environmental Control

### Before: Complex scripting of hardware
Coordinating temperature controllers, CO2 mixers, autofocus systems, and time-lapse acquisition requires intricate scripting and timing.

### After: CUBE Protocol
The logic is expressed semantically.
...
\`\`\`
`,
    'RECURSION.md': `# THE CUBE PROTOCOL ALGORITHM - COMPLETE IMPLEMENTATION
**The Universal Compression & Control Algorithm**
