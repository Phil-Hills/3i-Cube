...
import { PhotoIcon, CubeIcon, CalendarIcon, TagIcon } from './icons';

interface GalleryViewProps {
  images: GalleryImage[];
  onImageSelect: (image: { imageUrl: string; cubeScript: string; id: number }) => void;
}
        ...
        <div className="col-span-12 md:col-span-3 bg-gray-950/40 backdrop-blur-2xl border border-white/10 rounded-lg p-4 flex flex-col overflow-y-auto">
            ...
            <div className="space-y-6">
                <div>
                 ...
                 <div>
                    <h3 className="text-md font-semibold text-cyan-300 mb-3 flex items-center"><CubeIcon className="w-5 h-5 mr-2"/>Microscope System</h3>
                    <div className="space-y-1">
                        {systems.map(sys => (
        ...
        <div className="col-span-12 md:col-span-9 overflow-y-auto">
            {filteredImages.length > 0 ? (
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                    {filteredImages.map(image => (
                        <div 
                            ...
                            onClick={() => onImageSelect({ imageUrl: image.imageUrl, cubeScript: image.cubeScript, id: image.id })}
                        >
                            <img src={image.imageUrl} alt={`Microscopy image from ${image.createdAt.toLocaleString()}`} className="w-full h-full object-cover rounded-lg bg-black/30" />
                            <div className="absolute inset-0 bg-black/70 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-end p-2 text-white text-xs">
                                <p className="font-bold">{image.tags.system} - {image.tags.technique}</p>
                                <p className="text-gray-400">{image.createdAt.toLocaleDateString()}</p>
                            </div>
                        </div>
                    ))}
                </div>
                ...
                <div className="flex flex-col items-center justify-center h-full text-gray-500 text-center">
                    ...
                    <p className="max-w-xs">Run experiments in the Executor and click "Save to Gallery" on the preview to start your collection.</p>
                </div>
            )}
        </div>
    </div>
